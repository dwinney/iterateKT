...
namespace iterateKT { namespace COMPASS
{
    ...
    struct fit
    {
        ...
        static const int kReal = 0, kImag = 1, kNorm = 2;
        static std::string data_type(int i)
        {
            switch (i)
            {
                case kReal: return "Re (A)";
                case kImag: return "Im (A)";
                case kNorm: return "|A|^2";
        ...
        static double fcn(std::vector<data_set> & data_vector, amplitude to_fit)
        {
            ...
            for (auto data : data_vector)
            {
                for (int i = 0; i < data._N; i++)
                {
                    ...
                    double u = to_fit->get_kinematics()->Sigma() - s - t;
                    ...
                    switch (data._type)
                    {
                        ...
                        case kNorm: chi2 += norm(from_data - norm(from_model)); break;
                        default: break;
                    };
                };
            };
    ...
    inline std::array<data_set,2>  parse_JSON(std::string input)
    {
        // Final outputs
        ...
        auto bin_centers = data["bin_centers"].template get<std::vector<double>>();
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                ...
                re.push_back(real_parts[i][j]);
                im.push_back(imag_parts[i][j]);
            };
        };
