...
void test_bounds()
{
    ...
    p1.add_curve({sth, 1}, [&](double s){   return std::real(omega->t_plus(s));},  "#it{t}_{#plus}(#it{s})");
    p1.add_curve({sth, 1}, [&](double s){   return std::real(omega->t_minus(s));}, "#it{t}_{#minus}(#it{s})");
    ...
    p1.set_labels("#it{s}   [GeV^{2}]", "Re #it{t}   [GeV^{2}]");

    plot p2 = plotter.new_plot();
    p2.set_curve_points(200);
    p2.add_curve({sth, 1}, [&](double s){ return std::imag(omega->t_plus(s));});
    p2.add_curve({sth, 1}, [&](double s){ return std::imag(omega->t_minus(s));});
    ...
    p2.set_labels("#it{s}   [GeV^{2}]", "Im #it{t}   [GeV^{2}]");
