// Calculate partial widths for K -> 3π using parameters in [1]
//
// ------------------------------------------------------------------------------
...
void calculate_observables()
{
    ...
    std::array<double,3> zzc = {M_PION_0,  M_PION_0,  M_PION_PM};
    std::array<double,3> ccz = {M_PION_PM, M_PION_PM, M_PION_0};  

    line(); divider();
    print("K⁺ → π⁺π⁺π⁻"); divider(2);
    amp->set_option(option::p_ppm);
    dpars = amp->get_dalitz_parameters(eps, M_PION_PM*M_PION_PM);
    print<20>("Γ (× 10¹⁸)", width_with_physical_masses(amp, M_KAON_PM, ccc));
    print<15>("g", dpars[0]);
    print<15>("h", dpars[1]);
    print<15>("k", dpars[3]);
    line();
    ...
    dpars = amp->get_dalitz_parameters(eps, M_PION_PM*M_PION_PM);
    print<20>("Γ (× 10¹⁸)", width_with_physical_masses(amp, M_KAON_PM, zzc));
    print<15>("g", dpars[0]);
    print<15>("h", dpars[1]);
    print<15>("k", dpars[3]);
    line();
    
    print("KL → π⁺π⁻π⁰"); divider(2);
    amp->set_option(option::L_pmz);
    dpars = amp->get_dalitz_parameters(eps, M_PION_PM*M_PION_PM);
    print<20>("Γ (× 10¹⁸)", width_with_physical_masses(amp, M_KAON_0, ccz));
    print<15>("g", dpars[0]);
    print<15>("h", dpars[1]);
    print<15>("k", dpars[3]);
    line();
    ...
    dpars = amp->get_dalitz_parameters(eps, M_PION_PM*M_PION_PM);
    print<20>("Γ (× 10¹⁸)", width_with_physical_masses(amp, M_KAON_0, zzz));
    print<15>("h", dpars[1]);
    divider();
};